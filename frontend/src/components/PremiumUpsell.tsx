...
import { Star, Zap, Infinity as InfinityIcon, ShieldCheck } from 'lucide-react';
...
export const PremiumUpsell: React.FC = () => {
  ...
  return (
    <div className="glass-card flex flex-col gap-4 border border-[var(--accent-blue)]/50 relative overflow-hidden">
      <div className="absolute top-0 right-0 w-32 h-32 bg-sky-500/10 blur-3xl -z-10 rounded-full" />
      
      <div className="flex items-center gap-2">
        <Star className="text-sky-400 fill-sky-400/20" size={24} />
        <h3 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-blue-400">
          ProofNode Premium
        </h3>
      </div>
      ...
      <ul className="flex flex-col gap-2 my-2 text-sm">
        <li className="flex items-center gap-2">
          <InfinityIcon size={16} className="text-[var(--accent-green)]" />
      ...
      <div className="flex gap-2 mt-2">
        <button 
          className="flex-1 py-3 px-4 bg-gradient-to-r from-sky-500 to-blue-500 rounded-xl font-bold text-sm shadow-lg shadow-sky-500/20 active:scale-95 transition-all text-white border-transparent"
